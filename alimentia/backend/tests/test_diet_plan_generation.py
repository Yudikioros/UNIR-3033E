...
class PromptContentTests(unittest.TestCase):
    def test_prompt_incluye_requerimientos_calculados(self):
        context = _context()
        _system, user_prompt = gen.build_prompt(
            context, [], [], food_available=False, knowledge_available=False)
    ...
    def test_prompt_no_solicita_recalcular(self):
        context = _context()
        system_prompt, user_prompt = gen.build_prompt(
            context, [], [], food_available=False, knowledge_available=False)
    ...
    def test_prompt_exige_autoverificacion_y_tolerancias(self):
        ...
        system_prompt, user_prompt = gen.build_prompt(
            context, [], [], food_available=False, knowledge_available=False)
        self.assertIn('AUTOVERIFICACIÓN OBLIGATORIA', system_prompt)
        self.assertIn('±5%', user_prompt)
        self.assertIn('±10%', user_prompt)
    ...
    def test_food_database_disponible_incluye_datos_verificados(self):
        ...
        food = types.SimpleNamespace(
            name='POLLO PECHUGA', energyKcal=165.0, proteinG=31.0, fatG=3.6, carbohydratesG=0.0)
        _system, user_prompt = gen.build_prompt(
    ...
    def test_rag_vacio_en_prompt(self):
        ...
        _system, user_prompt = gen.build_prompt(
            context, [], [], food_available=False, knowledge_available=False)
        self.assertIn(
            'No hay documentos de conocimiento clínico autorizados', user_prompt)
        self.assertIn('No cites ninguna fuente', user_prompt)

    def test_rag_con_fuente_en_prompt(self):
        context = _context()
        chunk = types.SimpleNamespace(
            documentName='Guía INSP', content='Contenido de referencia sobre fibra.')
        _system, user_prompt = gen.build_prompt(
            context, [], [chunk], food_available=False, knowledge_available=True)
...
class PrivacyTests(unittest.TestCase):
    ...
    def test_contexto_no_admite_campos_de_identidad(self):
        ...
        self.assertTrue(forbidden.isdisjoint(
            DietPlanGenerationContext.model_fields.keys()))
    ...
    def test_build_context_no_filtra_pii_del_paciente(self):
        consultation = types.SimpleNamespace(
            ...
            patient=types.SimpleNamespace(
                name='María González', email='maria.gonzalez@example.com', phone='555-0100'),
        )
...
class ValidationTests(unittest.TestCase):
    def test_meal_count_mismatch(self):
        ...
        validations = gen.run_validations(
            context, plan, food_available=True, knowledge_available=True)
        codes = {v['code'] for v in validations}
        self.assertIn('MEAL_COUNT_MISMATCH', codes)
        mismatch = next(
            v for v in validations if v['code'] == 'MEAL_COUNT_MISMATCH')
    ...
    def test_restricted_food_found(self):
        ...
        plan = _plan(
            [_meal('Colación', [_food('Ensalada con nueces', calories=200)])])
        validations = gen.run_validations(
            context, plan, food_available=True, knowledge_available=True)
        restricted = [v for v in validations if v['code']
                      == 'RESTRICTED_FOOD_FOUND']
    ...
    def test_restricted_allergy_found(self):
        ...
        plan = _plan(
            [_meal('Colación', [_food('Crema de cacahuate', calories=200)])])
        validations = gen.run_validations(
            context, plan, food_available=True, knowledge_available=True)
        self.assertTrue(
            any(v['code'] == 'RESTRICTED_FOOD_FOUND' for v in validations))

    def test_energy_within_tolerance(self):
        context = _context(mealsPerDay=1, targetCalories=1000.0)
        # 3% de desviación
        plan = _plan([_meal('Comida', [_food('Alimento', calories=1030.0)])])
        validations = gen.run_validations(
            context, plan, food_available=True, knowledge_available=True)
        codes = {v['code'] for v in validations}
    ...
    def test_energy_out_of_tolerance(self):
        context = _context(mealsPerDay=1, targetCalories=1000.0)
        # 20% de desviación
        plan = _plan([_meal('Comida', [_food('Alimento', calories=1200.0)])])
        validations = gen.run_validations(
            context, plan, food_available=True, knowledge_available=True)
        codes = {v['code'] for v in validations}
        self.assertIn('ENERGY_OUT_OF_TOLERANCE', codes)
        out_of_tolerance = next(
            v for v in validations if v['code'] == 'ENERGY_OUT_OF_TOLERANCE')
    ...
    def test_incomplete_nutrition_data_no_calcula_desviacion(self):
        ...
        plan = _plan(
            [_meal('Comida', [_food('Alimento sin calorías', calories=None)])])
        validations = gen.run_validations(
            context, plan, food_available=True, knowledge_available=True)
        codes = {v['code'] for v in validations}
        ...
        self.assertNotIn('ENERGY_OUT_OF_TOLERANCE', codes)
        incomplete = next(
            v for v in validations if v['code'] == 'INCOMPLETE_NUTRITION_DATA')
    ...
    def test_food_database_unavailable_validation(self):
        ...
        validations = gen.run_validations(
            context, plan, food_available=False, knowledge_available=True)
        self.assertTrue(any(
            v['code'] == 'FOOD_DATABASE_UNAVAILABLE' and not v['isBlocking'] for v in validations))

    def test_knowledge_base_unavailable_validation(self):
        context = _context(mealsPerDay=1)
        plan = _plan([_meal('Comida', [_food('Alimento', calories=1000.0)])])
        validations = gen.run_validations(
            context, plan, food_available=True, knowledge_available=False)
        self.assertTrue(any(
            v['code'] == 'KNOWLEDGE_BASE_UNAVAILABLE' and not v['isBlocking'] for v in validations))
