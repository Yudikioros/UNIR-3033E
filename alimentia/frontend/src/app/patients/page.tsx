    ...
    Promise.all([
      ...
      .catch((err) => {
        if (!controller.signal.aborted)
          setError(errorMessage(err, "No fue posible cargar los pacientes."));
      });
  ...
  return (
    <div className="flex h-full flex-1 flex-col overflow-hidden bg-[#F8FAFC]">
      <header className="flex shrink-0 items-center justify-between gap-4 border-b border-slate-200 bg-white px-8 py-5">
        ...
        <Link href="/patients/new" className={primaryButton}>
      ...
      <div className="custom-scrollbar flex-1 overflow-y-auto p-8">
        <div className="mx-auto max-w-7xl">
          {error ? (
            ...
            <PatientTable patients={data.patients} options={data.options} />
          ) : (
            <p
              role="status"
