...
const generationSessionKey = (consultationId: string) =>
  `alimentia:generation:${consultationId}`;
...
const dangerButton =
  "inline-flex items-center justify-center rounded-lg border border-red-200 px-3 py-2 text-xs font-semibold text-red-600 hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-50";
const dangerSolidButton =
  "inline-flex items-center justify-center rounded-lg bg-red-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-red-700 disabled:cursor-not-allowed disabled:opacity-50";
const inputClass =
...
function toEditMeals(plan: DietPlanDetail): DietPlanMealEdit[] {
  return plan.meals.map((meal) => ({
    ...
    foods: meal.foods.map((food) => ({
      ...
      unit: food.unit ?? "",
      smaeEquivalent: food.smaeEquivalent,
...
export function DietPlanDraft({
  ...
  useEffect(() => {
    ...
    getConsultationPlans(consultation.id, controller.signal)
      .then((list) => {
        setVersions(list);
        const requestedIndex = initialPlanId
          ? list.findIndex((plan) => plan.id === initialPlanId)
          : -1;
        setSelected(
          list.length
            ? requestedIndex >= 0
      ...
      .catch((err) => {
        if (!controller.signal.aborted)
          setLoadError(errorMessage(err, "No fue posible cargar el plan."));
      });
    ...
    const storedId = sessionStorage.getItem(
      generationSessionKey(consultation.id),
    );
    if (storedId) {
      getGenerationStatus(storedId)
        .then(async (status) => {
          ...
          setJob(status);
          const final = await pollGenerationStatus(
            storedId,
            () => cancelled,
            setJob,
          );
  ...
  const blockedToGenerate =
    !consultation || !calculated || consultation.readinessIssues.length > 0;
  const current =
    versions && selected !== undefined ? versions[selected] : undefined;
  ...
  async function generate() {
    ...
    try {
      ...
      setActionError(
        errorMessage(
          err,
          "No fue posible generar el borrador. El proveedor puede tardar varios minutos o no estar disponible; intenta nuevamente.",
        ),
      );
      setJob(undefined);
    } finally {
  ...
  function updateFood(
    ...
    patch: Partial<DietPlanMealEdit["foods"][number]>,
  ) {
    setEditMeals((meals) =>
      meals.map((meal, i) =>
        i !== mi
  ...
  function removeFood(mi: number, fi: number) {
    setEditMeals((meals) =>
      meals.map((meal, i) =>
        i !== mi
          ? meal
          : { ...meal, foods: meal.foods.filter((_, j) => j !== fi) },
      ),
    );
  }
    ...
    setEditMeals((meals) =>
      meals.map((meal, i) =>
        i !== mi
          ...
          : {
              ...meal,
              foods: [...meal.foods, { foodName: "", quantity: 1, unit: "g" }],
            },
      ),
    );
  }
  function updateMeal(mi: number, patch: Partial<DietPlanMealEdit>) {
    setEditMeals((meals) =>
      meals.map((meal, i) => (i !== mi ? meal : { ...meal, ...patch })),
    );
  }
  ...
  function addMeal() {
    setEditMeals((meals) => [
      ...meals,
      {
        mealType: "Comida",
        name: "Nueva comida",
        foods: [{ foodName: "", quantity: 1, unit: "g" }],
      },
    ]);
  }
  ...
  async function saveEdit() {
    ...
    if (!cleaned.length) {
      setActionError("El plan debe tener al menos una comida con un alimento.");
      return;
    }
    setActionBusy(true);
    setActionError("");
    try {
      await editPlan(current.id, {
        meals: cleaned,
  ...
  async function doRegenerate() {
    ...
    try {
      const { generationId } = await startRegeneratePlan(
        current.id,
      ...
      setInstructions("");
      sessionStorage.setItem(
        generationSessionKey(current.consultationId),
        generationId,
      );
      const final = await pollGenerationStatus(
        generationId,
      ...
      setActionError(
        errorMessage(
          err,
          "No fue posible generar una nueva versión. Intenta nuevamente.",
        ),
      );
      setRegenJob(undefined);
    } finally {
  ...
  if (!consultation) {
    return (
      <p className="py-8 text-center text-sm text-slate-400">
        Abre una consulta para generar un borrador.
      </p>
    );
  }
    ...
    <div className="space-y-4">
      ...
      {!calculated ? (
        <p className="text-xs text-slate-500">
          Calcula los requerimientos nutricionales antes de generar el borrador.
        </p>
      ) : null}
        ...
        <div className="space-y-3">
          {!job ? (
            <button
              ...
              onClick={generate}
              disabled={generating || blockedToGenerate}
            >
            ...
            <>
              ...
              {regenJob ? (
                <GenerationProgress
                  ...
                  stage={regenJob.stage}
                  startedAt={regenJob.startedAt}
                  error={regenJob.errorMessage}
                  onRetry={retryRegenerate}
                />
              ) : null}
              ...
              {current.isEditable ? (
                <div className="flex flex-wrap gap-3 border-t border-slate-100 pt-4">
                  ...
                  >
                    Aprobar plan
                    {current.blockingValidationCount
                      ? ` (${current.blockingValidationCount} pendiente${current.blockingValidationCount > 1 ? "s" : ""})`
                      : ""}
                  </button>
                  <button
    ...
    return (
      <div className="rounded-lg border border-green-200 bg-green-50 px-4 py-3">
        <p className="text-xs font-bold uppercase tracking-wide text-green-700">
          APROBADO · versión {plan.version}
        </p>
        <p className="text-xs text-green-700">
          Revisado y aprobado por profesional
          {plan.approvedBy ? ` (${plan.approvedBy})` : ""}
          {plan.approvedAt
            ? ` · ${new Date(plan.approvedAt).toLocaleDateString("es-MX", { day: "2-digit", month: "short", year: "numeric" })}`
            : ""}
        </p>
      </div>
    );
    ...
    return (
      <div className="rounded-lg border border-red-200 bg-red-50 px-4 py-3">
        <p className="text-xs font-bold uppercase tracking-wide text-red-700">
          RECHAZADO · versión {plan.version}
        </p>
        <p className="text-xs text-red-700">
          {plan.rejectionReason}
          {plan.rejectedAt
            ? ` · ${new Date(plan.rejectedAt).toLocaleDateString("es-MX", { day: "2-digit", month: "short", year: "numeric" })}`
            : ""}
        </p>
      </div>
    );
...
function PlanSummaryPanel({ plan }: { plan: DietPlanDetail }) {
  const deviation =
    plan.totalCalories != null && plan.targetCalories
      ? ((plan.totalCalories - plan.targetCalories) / plan.targetCalories) * 100
      : null;
  ...
  return (
    <div className="grid grid-cols-2 gap-3 rounded-lg border border-slate-200 bg-white p-4 sm:grid-cols-4">
      {cell(
        "Fecha de generación",
        plan.generatedAt ? dateLabel(plan.generatedAt) : "—",
      )}
      {cell(
        "Energía objetivo",
        plan.targetCalories != null
          ? `${Math.round(plan.targetCalories)} kcal`
          : "—",
      )}
      {cell(
        "Energía del plan",
      ...
      <div>
        ...
        <p
          className={`text-xs font-semibold ${deviation == null ? "text-slate-700" : Math.abs(deviation) <= 5 ? "text-green-600" : "text-red-600"}`}
        >
          {deviation != null
      ...
      {cell(
        ...
        plan.proteinGrams != null
          ? `${plan.proteinGrams} g${plan.targetProteinGrams != null ? ` / ${Math.round(plan.targetProteinGrams)} g obj.` : ""}`
          : "—",
      )}
      {cell(
        "Carbohidratos",
        plan.carbohydrateGrams != null
          ? `${plan.carbohydrateGrams} g${plan.targetCarbohydrateGrams != null ? ` / ${Math.round(plan.targetCarbohydrateGrams)} g obj.` : ""}`
          : "—",
      )}
      {cell(
        "Grasa",
        plan.fatGrams != null
          ? `${plan.fatGrams} g${plan.targetFatGrams != null ? ` / ${Math.round(plan.targetFatGrams)} g obj.` : ""}`
          : "—",
      )}
      {cell(
        "Fibra / Agua",
        `${plan.fiberGrams != null ? `${plan.fiberGrams} g` : "—"} / ${plan.waterLiters != null ? `${plan.waterLiters} L` : "—"}`,
      )}
    </div>
  );
}
...
function ValidationsList({
  ...
  validations: DietPlanDetail["validations"];
}) {
  if (!validations.length) return null;
  const blocking = validations.filter((validation) => validation.isBlocking);
  const warnings = validations.filter(
    (validation) => !validation.isBlocking && validation.severity === "WARNING",
  );
  const info = validations.filter(
    (validation) => !validation.isBlocking && validation.severity !== "WARNING",
  );
  return (
    <div className="space-y-2">
      ...
      {blocking.length ? (
        <Notice error>
          Este plan no puede aprobarse hasta corregir las validaciones
          bloqueantes.
        </Notice>
      ) : null}
...
function ValidationGroup({
  ...
  title: string;
  items: DietPlanDetail["validations"];
}) {
  ...
  return (
    <div className="space-y-1">
      ...
      {items.map((validation) => (
        <div
          key={validation.id}
          className={`rounded-lg border px-3 py-2 text-xs ${SEVERITY_STYLE[validation.severity]}`}
        >
          <span className="font-bold">{validation.severity}: </span>
          {validation.message}
          {SEVERITY_HINT[validation.severity]}
        </div>
      ))}
    </div>
  );
...
function SourcesBlock({ plan }: { plan: DietPlanDetail }) {
  const knowledgeUnavailable = plan.validations.some(
    (v) => v.code === "KNOWLEDGE_BASE_UNAVAILABLE",
  );
  const foodUnavailable = plan.validations.some(
    (v) => v.code === "FOOD_DATABASE_UNAVAILABLE",
  );
  return (
    <div className="space-y-1">
      ...
      {plan.sources.length ? (
        ...
        <p className="text-xs text-slate-500">
          Ninguna fuente documental configurada.
        </p>
      )}
      {knowledgeUnavailable ? (
        <p className="text-xs text-amber-700">
          La base de conocimiento documental aún no está configurada. El
          borrador fue generado sin contexto RAG.
        </p>
      ) : null}
      {foodUnavailable ? (
        <p className="text-xs text-amber-700">
          La base alimentaria estructurada aún no está configurada.
        </p>
      ) : null}
    </div>
  );
...
function ExportAndTraceability({ plan }: { plan: DietPlanDetail }) {
  ...
  async function onExport() {
    ...
    try {
      ...
      setExportError(
        errorMessage(err, "No fue posible exportar el plan a PDF."),
      );
    } finally {
      setExporting(false);
    }
  ...
  async function onToggleTrace() {
    ...
    try {
      ...
      setTraceError(
        errorMessage(err, "No fue posible cargar la trazabilidad."),
      );
    }
  }
  ...
  return (
    <div className="space-y-3 border-t border-slate-100 pt-4">
      ...
      {showTrace ? (
        traceError ? (
          <Notice error>{traceError}</Notice>
        ) : !trace ? (
          ...
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-xs text-slate-600 space-y-1">
            <p>
              ...
              {trace.calculation.method ?? "—"} (ruleset{" "}
              {trace.calculation.rulesetVersion ?? "—"})
            </p>
            <p>
            ...
            <p>
              ...
              {trace.resources.foodDatabaseUsed
                ? `usado (${trace.resources.foodDatabaseName} ${trace.resources.foodDatabaseVersion})`
                : "no usado"}{" "}
              · RAG {trace.resources.knowledgeBaseUsed ? "usado" : "no usado"}
            </p>
            <p>
              <span className="font-semibold">Intervención humana:</span>{" "}
              {trace.humanReview.manualEditCount} edición(es) ·{" "}
              {trace.humanReview.regenerationCount} regeneración(es)
              solicitada(s)
            </p>
...
function EditForm({
  ...
  return (
    <div className="space-y-4 rounded-lg border border-blue-200 bg-blue-50/40 p-4">
        ...
        >
          <div className="flex flex-wrap items-center gap-2">
            <input
              ...
              placeholder="Tipo"
              onChange={(event) =>
                onUpdateMeal(mi, { mealType: event.target.value })
              }
            />
            <input
              className={inputClass}
              ...
              onChange={(event) =>
                onUpdateMeal(mi, { name: event.target.value })
              }
            />
            <button
              type="button"
              className="text-xs font-semibold text-red-600"
              onClick={() => onRemoveMeal(mi)}
            >
              Eliminar comida
            </button>
          </div>
          <div className="space-y-2">
            {meal.foods.map((food, fi) => (
              <div
                key={fi}
                className="grid grid-cols-2 gap-2 rounded border border-slate-100 p-2 sm:grid-cols-6"
              >
                <input
                  ...
                  onChange={(event) =>
                    onUpdateFood(mi, fi, { foodName: event.target.value })
                  }
                />
                <input
                  className={inputClass}
                ...
                <input
                  ...
                  placeholder="Unidad"
                  onChange={(event) =>
                    onUpdateFood(mi, fi, { unit: event.target.value })
                  }
                />
                <input
                  className={inputClass}
      ...
      <button type="button" className={secondaryButton} onClick={onAddMeal}>
        Agregar comida
      </button>
  ...
  return (
    <div className="space-y-3 rounded-lg border border-slate-200 bg-slate-50 p-4">
      <p className="text-xs text-slate-700">
        Al aprobar este plan quedará registrado como versión final revisada por
        el profesional.
      </p>
    ...
    <div className="space-y-3 rounded-lg border border-slate-200 bg-slate-50 p-4">
      <label className="block text-xs font-semibold text-slate-600">
        ...
        <textarea
          ...
          rows={2}
          maxLength={500}
          placeholder="Ej. Evitar lácteos, usar preparaciones más sencillas"
          value={instructions}
          onChange={(event) => onChange(event.target.value)}
        />
      </label>
      <div className="flex gap-3">
        <button className={primaryButton} onClick={onConfirm} disabled={busy}>
          {busy ? "Generando nueva versión…" : "Generar nueva versión"}
        </button>
        <button className={secondaryButton} onClick={onCancel} disabled={busy}>
          Cancelar
        </button>
