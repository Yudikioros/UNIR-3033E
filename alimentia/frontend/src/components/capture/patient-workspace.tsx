...
export function PatientWorkspace({
  ...
  useEffect(() => {
    ...
    Promise.all([
      getPatient(id, controller.signal),
      getPatientConsultations(id, controller.signal),
      getCaptureOptions(controller.signal),
      activeId && activeId !== "new"
        ? getConsultation(activeId, controller.signal)
        : Promise.resolve(undefined),
    ])
      .then(([patient, consultations, options, active]) => {
        if (active && active.patientId !== id) {
          setError("Esta consulta no pertenece al paciente seleccionado.");
          return;
        }
        ...
        if (!controller.signal.aborted)
          setError(
            err instanceof ApiError && err.status === 404
              ? "No se encontró el paciente o la consulta."
              : errorMessage(
                  err,
                  "No fue posible cargar los datos del paciente.",
                ),
          );
      });
  ...
  function navigate(event: React.MouseEvent<HTMLAnchorElement>) {
    if (dirty) {
      event.preventDefault();
      setNotice("Guarda los cambios pendientes antes de cambiar de consulta.");
    }
  }
  ...
  function consultationSaved(consultation: Consultation) {
    ...
    setData((current) =>
      current
        ? {
            ...
            consultations: [
              consultation,
              ...current.consultations.filter((c) => c.id !== consultation.id),
            ].sort((a, b) =>
    ...
    if (activeId !== consultation.id)
      router.replace(
        `/patients/${id}?consultation=${encodeURIComponent(consultation.id)}&saved=1`,
      );
  }
    ...
    setData((current) =>
      current
        ? {
            ...current,
            active: consultation,
            consultations: current.consultations.map((c) =>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-800">
              {data ? `Paciente: ${data.patient.name}` : "Ficha del paciente"}
            </h1>
            <p className="mt-1 text-xs text-slate-500">
              Datos del paciente e historial de consultas
            </p>
          </div>
        </div>
        {data ? (
          <Link
            href={`/patients/${id}?consultation=new`}
            onClick={navigate}
      ...
      <div className="custom-scrollbar flex-1 overflow-y-auto p-8">
        {error ? (
          <div className="space-y-4">
            ...
            <Link href="/patients" className="ml-4 text-sm text-blue-600">
              Volver a Pacientes
            </Link>
          </div>
        ) : !data ? (
            ...
            <div className="grid grid-cols-1 items-start gap-6 pb-8 xl:grid-cols-2">
              <div className="space-y-6">
                <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                  <h2 className="mb-6 text-sm font-bold text-blue-600">
                    1. Datos del paciente
                  </h2>
                  {data.patient.isDemo ? (
                    <p className="mb-4 text-xs text-blue-600">
                      Caso demostrativo del MVP
                    </p>
                  ) : null}
                  <PatientForm
                    key={data.patient.updatedAt}
                    patient={data.patient}
                    options={data.options}
                    onSaved={patientSaved}
                    onDirtyChange={setPatientDirty}
                  />
                </section>
                          ...
                          data.consultations.map((consultation) => (
                            ...
                            >
                              <td className="px-3 py-4">
                                {dateLabel(consultation.consultationDate)}
                              </td>
                              <td className="px-3 py-4">
                              ...
                              <td className="px-3 py-4">
                                {consultation.plans.length
                                  ? consultation.plans.map((plan) => (
                                      <span
                                        key={plan.id}
                                        className="block"
                                        title={plan.id}
                              ...
                              <td className="px-3 py-4">
                                <Link
                                  href={`/patients/${id}?consultation=${encodeURIComponent(consultation.id)}`}
                                  onClick={navigate}
              ...
              <div className="space-y-6">
                <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                  <h2 className="mb-6 text-sm font-bold text-blue-600">
                    2. Datos de la consulta actual
                  </h2>
                  {activeId ? (
                    <ConsultationForm
                      key={data.active?.updatedAt || "new"}
                      patient={data.patient}
                      consultation={data.active}
                      options={data.options}
                      onSaved={consultationSaved}
                      onDirtyChange={setConsultationDirty}
                    />
                  ) : (
                    <div className="py-12 text-center text-sm text-slate-400">
                      Abre una consulta del historial o selecciona Nueva
                      consulta.
                    </div>
                  )}
                </section>
